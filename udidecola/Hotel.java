...
class Hotel extends Empresa // Item 1f) registro de hotéis
{
	...
	private String desc;
	private final static double taxa = 0.05; // Item 1p) taxa que será aplicada de acordo com valor para a UdiDecola e
												// Item 5)
	...
	public Hotel(String CNPJ, String nome, String nomeDivulg, String endLogradouro, int diaCria, int mesCria,
			...
			int numEstrelas, boolean accPet, int numQuartos, String msgDivulg, String desc, int hCheckin,
			int minCheckin, int segCheckin, int hCheckout, int minCheckout, int segCheckout) {

		super(CNPJ, nome, nomeDivulg, endLogradouro, diaCria, mesCria, anoCria, endNumero, endBairro, endCidade, endUf,
				endCep);
	...
	public void registrarQuarto(double diariaSdesc, double desconto, double diariaCdesc, int quantidade, String tipo) // Item
																														// 1g)
																														// e
																														// 1h)
	{
		Quarto quarto = new Quarto(diariaSdesc, desconto, quantidade, tipo); // Função feita para o registro de quartos
																				// pelo hotel.
	...
	public boolean CancelamentoAceito(Date data) { // Item 1g) política de cancelamento
		...
		return cal1.get(Calendar.YEAR) == cal2.get(Calendar.YEAR)
				...
				&& cal1.get(Calendar.DAY_OF_MONTH) == cal2.get(Calendar.DAY_OF_MONTH) && cancelamentoAceito;
	}
